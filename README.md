...
Utsuwa can talk to any local TTS server that exposes an OpenAI-compatible `/audio/speech` endpoint.

- Set the **API base URL** to your server, for example `http://localhost:8880/` (Kokoro-FastAPI) or `http://localhost:8000/` (openedai-speech)
- Pick a **voice** from the dropdown (loaded from the server's `/audio/voices` endpoint if available)
- The spoken language is determined by the selected voice/model on the server side

This provider is useful for self-hosted, offline TTS without provider-specific code paths.
...
- [x] Desktop application with transparent overlay mode (macOS, Windows, Linux)
...
- [ ] **Advanced Desktop Integration** - Deeper OS integrations for overlay, notifications, and system shortcuts
